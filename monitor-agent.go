...
func sendData(monitorHost string, appId string, taskId string, resp *RestcommResponse) {
	Trace.Println("Send data: ", appId, ";", taskId)
	load := int(float64(resp.Metrics.LiveCalls) / float64(restcommMaxCalls) * 100)

	data := make(map[string]interface{})
	data["_TaskId"] = taskId
	data["_LoadCalls"] = load
	data["_LiveCalls"] = resp.Metrics.LiveCalls
	data["_LiveOutgoingCalls"] = resp.Metrics.LiveOutgoingCalls
	data["_LiveIncomingCalls"] = resp.Metrics.LiveIncomingCalls
	data["_TotalCallsSinceUptime"] = resp.Metrics.TotalCallsSinceUptime
	data["_CompletedCalls"] = resp.Metrics.CompletedCalls
	data["_FailedCalls"] = resp.Metrics.FailedCalls

	msg := &gelf.Message{Extra: data, Version: "1.1", Host: appId, Short: appId, Full: resp.InstanceId, Level: 6}
	gelfWriter.WriteMessage(msg)
...
func main() {
	...
	writer, err := gelf.NewWriter(*monitorHost)
	if err != nil {
		Error.Println("gelf.NewWriter:", err)
		panic("can not connect to graylog")
	}
	gelfWriter = writer
	do := func() {
